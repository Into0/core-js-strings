...
function removeFirstOccurrences(str, value) {
  ...
  return i !== -1 ? str.substring(0, i) + str.substring(i + value.length) : str;
}
...
function formatTime(minutes, seconds) {
  const timeFormat = (zero) => zero.padStart(2, '0');
  return `${timeFormat(String(minutes))}:${timeFormat(String(seconds))}`;
}
...
function isPalindrome(str) {
  const p = str.replace(/[.!?]/, '').toLowerCase();
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (char) => cipher[alpha.indexOf(char)]);
}
